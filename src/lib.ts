...
function isNewLine(node: ElementContent): boolean {
    ...
    return {
        ...
        code(node: Element) {
            ...
            for (let i = 0; i < node.children.length; i++) {
                ...
                const newline = node.children[i + 1];

                if (!isNewLine(newline)) { // last line handled
                    continue;
                }
                if (!isElement(line)) {
                    continue;
                }
                // search if line contains //^
                ...
                console.log(match[1]);
                // console.log(line, readText(line));
            }
