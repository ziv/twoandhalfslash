...
function ellipse(width: number, height: number, strokeWidth: number, stroke: string, x: string, y: string): Element {
    const rx = width / 2;
    const ry = height / 2;
    return element("svg",
        [
...
function transformFloating(el: Element, args: string[], dir: 'up' | 'down' | 'right' | 'left' = 'up') {
...
function trandformEllipse(el: Element, args: string[]) {
